<?php
/**
 * AdministrateursApiInterface
 * PHP version 5
 *
 * @category Class
 * @package  Swagger\Server
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 */

/**
 * BoulangerieApi
 *
 * The Boulangerie finally debugged for use, hosting service might need to be adapted
 *
 * OpenAPI spec version: 1.0.0
 * Contact: leafyteam@notaRealCompany.com
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 *
 */

/**
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen
 * Do not edit the class manually.
 */

namespace Swagger\Server\Api;

use Symfony\Component\HttpFoundation\File\UploadedFile;
use Swagger\Server\Model\Administrateurs;

/**
 * AdministrateursApiInterface Interface Doc Comment
 *
 * @category Interface
 * @package  Swagger\Server\Api
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 */
interface AdministrateursApiInterface
{

    /**
     * Operation addAdmin
     *
     * add a new Admin
     *
     * @param  Swagger\Server\Model\Administrateurs $body  Adminstrateur object that needs to be added (required)
     * @param  integer $responseCode     The HTTP response code to return
     * @param  array   $responseHeaders  Additional HTTP headers to return with the response ()
     *
     * @return void
     *
     */
    public function addAdmin(Administrateurs $body, &$responseCode, array &$responseHeaders);

    /**
     * Operation changeAdminPw
     *
     * changing Admin's password
     *
     * @param  string $loginAdmin   (required)
     * @param  string $passwordAdmin   (required)
     * @param  integer $responseCode     The HTTP response code to return
     * @param  array   $responseHeaders  Additional HTTP headers to return with the response ()
     *
     * @return void
     *
     */
    public function changeAdminPw($loginAdmin, $passwordAdmin, &$responseCode, array &$responseHeaders);

    /**
     * Operation deleteAdminbyLogin
     *
     * Deletes an Admin by login
     *
     * @param  string $loginAdmin  login of Admin to delete (required)
     * @param  integer $responseCode     The HTTP response code to return
     * @param  array   $responseHeaders  Additional HTTP headers to return with the response ()
     *
     * @return void
     *
     */
    public function deleteAdminbyLogin($loginAdmin, &$responseCode, array &$responseHeaders);

    /**
     * Operation getAdminById
     *
     * get Admin by Id
     *
     * @param  string $loginAdmin   (required)
     * @param  integer $responseCode     The HTTP response code to return
     * @param  array   $responseHeaders  Additional HTTP headers to return with the response ()
     *
     * @return Swagger\Server\Model\Administrateurs[]
     *
     */
    public function getAdminById($loginAdmin, &$responseCode, array &$responseHeaders);

    /**
     * Operation getAdministrateurs
     *
     * get detailsCommandesBL by Id
     *
     * @param  integer $responseCode     The HTTP response code to return
     * @param  array   $responseHeaders  Additional HTTP headers to return with the response ()
     *
     * @return Swagger\Server\Model\Administrateurs[]
     *
     */
    public function getAdministrateurs(&$responseCode, array &$responseHeaders);
}
